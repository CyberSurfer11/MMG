...
def train_multi_agents(
    ...
    for rnd in range(1, max_rounds+1):
        ...
        while not all(dones) and step < max_steps:
            ...
            for idx, ag in enumerate(agents):
                ...
                # === NEW: 记录本步数据（每个 agent 一行）
                try:
                    action_vec = actions[idx]
                    # 将可能的向量/ndarray统一转为列表并序列化为 JSON 字符串，方便 Excel 中查看
                    action_json = json.dumps(np.array(action_vec).tolist(), ensure_ascii=False)
                    state_json  = json.dumps(np.array(states[idx]).tolist(), ensure_ascii=False)
                    next_state_json = json.dumps(np.array(full_state).tolist(), ensure_ascii=False)
                except Exception:
                    # 兜底：转成字符串
                    action_json = str(actions[idx])
                    state_json = str(states[idx])
                    next_state_json = str(full_state)

                trace_rows.append({
                    "day": rnd,                   # 第几天/第几轮
                    "step": step,                 # 当天第几步
                    "agent": idx,                 # agent 编号
                    "state": state_json,          # 当前状态
                    "action": action_json,        # 动作
                    "next_state": next_state_json,# 下一状态
                    "done": bool(dones[idx]),     # 是否终止
                    "reward_step": float(new_r),  # 本步reward（未做1e-6缩放）
                    "penalty_step": float(info.get("penalty", 0.0)),
                    # 下面这些可帮助复盘（可选）
                    "P_n": float(P_n_record[idx]),
                    "C_n": float(C_n_record[idx]),
                    "elec_price_buy": float(np.array(e_p_m_buy).astype(float)),
                    "elec_price_sell": float(np.array(e_p_m_sell).astype(float)),
                    "carbon_price_buy": float(np.array(car_p_m_buy).astype(float)),
                    "carbon_price_sell": float(np.array(car_p_m_sell).astype(float)),
                })
                # === NEW END ===
    ...
    # === NEW: 统一导出到一个 Excel 文件（单 Sheet），用 day 列区分“每天/每轮”
    if trace_rows:
        df = pd.DataFrame(trace_rows)
        # 为了便于 Excel 里筛选，把列顺序略作整理
        preferred_cols = [
            "day","step","agent",
            "reward_step","penalty_step","done",
            "P_n","C_n",
            "elec_price_buy","elec_price_sell",
            "carbon_price_buy","carbon_price_sell",
            "state","action","next_state",
        ]
        cols = [c for c in preferred_cols if c in df.columns] + [c for c in df.columns if c not in preferred_cols]
        df = df[cols]
        df.to_excel("training_trace.xlsx", index=False)
        print(f"[保存] 训练全过程动作与状态已导出：training_trace.xlsx （使用 day 列区分每天）")
    # === NEW END ===
