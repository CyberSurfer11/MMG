...
class C_SAC_:
    def __init__(
        ...
        # critic 网络
        self.critic = self.network.critic(critic_units)
        self.critic_target = self.network.critic(critic_units)
    ...
    def compute_target_q(self, rewards, next_states, done_flags, constraint):
        ...
        # 2) 通过共享-head target critic 得到两个 Q
        nq1, nq2 = self.critic_target([next_states, sc])          # ← 修改
    ...
    def soft_update_all_targets(self):
        # 只更新两个 Critic (主+target) 和 Constraint Critic
    ...
    def _update_from_batch(self, batch):
        ...
        # compute_critic_loss 返回 q1_loss, q2_loss, q1, q2
