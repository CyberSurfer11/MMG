...
class Memory(object):  # stored as ( s, a, r, s_ ) in SumTree
    ...
    PyTorch compatible version with minor modifications for tensor handling
    """
    ...
    # 得到经验的索引、经验和IS权重 - PyTorch兼容版本
    def sample(self, n, device='cpu'):
        """
        采样经验，返回PyTorch tensors
        ...
        Returns:
            b_idx: 采样索引
            b_memory: 采样经验数据
            ISWeights: 重要性采样权重（PyTorch tensor）
        """
        ...
        for i in range(n):
            a_val, b_val = pri_seg * i, pri_seg * (i + 1)
        ...
        # 转换为PyTorch tensor
        ISWeights_tensor = torch.tensor(ISWeights, dtype=torch.float32, device=device)
        
        return b_idx, b_memory, ISWeights_tensor

    # 更新存储在优先级经验回放（Prioritized Experience Replay）内存中的经验的优先级。
    # 更新树中的优先级 - PyTorch兼容版本
    def batch_update(self, tree_idx, abs_errors):
        ...
        Args:
            tree_idx: 树节点索引
            abs_errors: 绝对误差，可以是numpy array或PyTorch tensor
        """
        # 如果是PyTorch tensor，转换为numpy
        if torch.is_tensor(abs_errors):
            abs_errors = abs_errors.detach().cpu().numpy()
        
        abs_errors = abs_errors.flatten()  # 确保是一维数组
...
if __name__ == "__main__":
    print("PyTorch compatible Prioritized Replay buffer test")
    ...
    # 添加一些测试数据
    for i in range(10):
        transition = np.array([i, i+1, i+2, i+3, i+4, i+5])  # 示例transition
    ...
    if len(memory) > 5:
        batch_size = 5
        indices, batch_data, is_weights = memory.sample(batch_size, device=device)
        print(f"Sampled batch shape: {batch_data.shape}")
        print(f"IS weights shape: {is_weights.shape}")
        print(f"IS weights device: {is_weights.device}")
        
        # 测试更新优先级
        errors = torch.randn(batch_size, device=device)
