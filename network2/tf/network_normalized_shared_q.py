...
class network:
    ...
    def critic(self, units=(128, 128, 32)):
        ...
        # 共享主干
        x = Dense(units[0], kernel_initializer=tf.keras.initializers.he_uniform(), name="Shared_L0")(concat)
        x = LeakyReLU()(x)
        x = LayerNormalization(name="Shared_LayerNorm0")(x)

        for index in range(1, len(units)):
            x = Dense(units[index], kernel_initializer=tf.keras.initializers.he_uniform(), name=f"Shared_L{index}")(x)
            x = LeakyReLU()(x)
            x = Dropout(0.1, name=f"Shared_Dropout{index}")(x)
        ...
        # 创建模型（共享主干 + 两个输出）
        critic_model = Model(inputs=inputs, outputs=[q1_output, q2_output])
